...
if not options.force and redis.get('opennms_salt_lock') is not None :
   log.warning( "Process %d is processing the queue, exiting"%(redis.get('opennms_salt_lock')) )
else:
   log.debug( "Locking the queue" )
   redis.set('opennms_salt_lock', pid)
   if redis.get('opennms_salt_lock') != pid :
      log.warning ( "Lock stolen by process with pid %d"%(redis.get('opennms_salt_lock')) )
      exit
   else:
...
for minion_id in redis.smembers(add_queue):
	try:
		# minion id
		...
		if requisition :
		    ...
		    if not dry_run:
		       redis.srem(add_queue, minion_id)
...
# remove minions from opennms
for minion_id in redis.smembers(delete_queue):
	try:
		...
		if not dry_run:
		   redis.srem(delete_queue, minion_id)
